...
void kinect_capture_app::do_glutIdle()
{
	...
	if(isRecording){
        ...
        fwrite(depth_frame->data, depthSize, 1, logFile);
        fwrite(video_frame->data, imageSize, 1, logFile);
...
void kinect_capture_app::init_glui() {
  ...
  new GLUI_Button(glui,"Record",0,&sset_record);
...
void kinect_capture_app::set_record(int id) {
